...
static uint8_t load_exec ()
{
	...
	// AG modified deleted code test debug
	/*
	...
	*/
	...
	while (!done) {
		...
		//printf("sr_buf[] = %x %x %x %x %x %x\n\r",sr_buf[0], sr_buf[1],sr_buf[2], sr_buf[3], sr_buf[4], sr_buf[5]);

		if ((ret = decode_srec_line (sr_buf, &srinfo)) != 0)
			return ret;

#ifdef VERBOSE
		// VERBOSE CUSTOM PLUS RAPIDE :
		/*
		if(compteur == 1000)
		{
			compteur = 0;
	...
	//laddr = xxxx; // custom modification de l'adresse ou on jump en quittant le bootloader.
	putnum ((uint32_t)laddr);
	print ("\r\n");
#endif
	(*laddr)(); // AG : JUMP TO THE APPLICATIF (adresse 0x0000000)
	// le bootloader se trouve pas au tout début mais une autre adresse vers 0x60000.

	// on ne doit jamais arriver ici sinon on a fait tout crasher.
	return 0;
}
